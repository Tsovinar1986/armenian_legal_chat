...
- Session tokens (`POST /api/auth/login`/`register` now issue one, validated via `GET /api/auth/me`) are additive/opt-in — no existing endpoint actually *requires* a valid token yet. Deliberately not enforced yet: retroactively requiring auth on previously-open endpoints (bookings, payments, etc.) is a product decision (which endpoints, and whether it breaks existing B2B integrations expecting no auth), not something to guess at in a code-only pass.
...
- The classifier-match and vector-match template responses in `LegalAgent.get_advice()` (Steps 2-3) are now also localized via the module-level `_TEMPLATE_TEXT`/`_t()` helper in `src/agents/legal_agent.py` — `hy`/`en` have real translations for every label (classification, similar case, recommended lawyer, etc.); any other requested code falls back to English, same convention as the crisis response.
